            ...
            <div class="form-group">
                ...
                <select name="application_id" id="application_id" class="form-control select2" onchange="changeApplication(this)">
                    ...
                    @foreach ($applications as $application)
                        <option value="{{ $application->encrypt_id }}">{{ $application->application_name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
...
<script>
    ...
    function changeApplication(params)
    {
        ...
        $.ajax({
            url:"{{url('master-apps/manage-menu/change-application')}}",
            type:'POST',
            beforeSend:function()
            {
    ...
    function addMenu()
    {
        ...
        $.ajax({
            url:"{{url('master-apps/manage-menu/add-new-menu')}}",
            type:'POST',
            beforeSend:function()
            {
                $('.loading-bar').removeClass('hidden');
