...
@section('custom-plugin')
    <script>
        ...
        var table = $('#user-permission-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ url('master-apps/manage-user-permission/get-data') }}",
        ...
        $('#user-permission-table tbody').on('click', 'td.details-control', function () {
            ...
            else
            {
                ...
                $('#table-data_'+user_id).DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: "{{ url('master-apps/manage-user-permission/get-data-application-permission') }}",
                        dataSrc: function ( json )
                        {
                            if(json.status!=='01' && json.status!='02') //!=false or !=token expire or !=token not valid
                            {
        ...
        function changeApplicationPermission(data)
        {
            ...
            $.ajax({
                url:"{{url('master-apps/manage-user-permission/change-application-permission')}}",
                type:'POST',
                beforeSend:function()
                {
        ...
        function menuPermissionModal(data)
        {
            ...
            $.ajax({
                url:"{{url('master-apps/manage-user-permission/menu-permission-modal')}}",
                type:'POST',
                beforeSend:function()
                {
                    $('.loading-bar').removeClass('hidden');
