                            ...
                            <div class="col-xl-10 col-lg-10 col-md-9 col-sm-9 col-9">
                                <select name="rpd_filling_another" id="rpd_filling_another" onchange="changeRPDForm(this.value)" class="form-control">
                                    @foreach ($rpd_filling_active as $list_rpd)
                                        <option value="{{ $list_rpd->encrypt_id }}" @if ($list_rpd->encrypt_id == $rpd_filling_head->encrypt_id) selected @endif> {{ $list_rpd->product->product_name }} </option>
                                    @endforeach
                                </select>
                            </div>

                        @else
                            RPD Filling Produk {{ $rpd_filling_head->product->product_name }}
                        @endif
                    </div>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                            <div class="form-group">
                                ...
                                <input type="text" name="product_name" id="product_name" class="form-control" value="{{ $rpd_filling_head->product->product_name }}" readonly>
                                <input type="hidden" name="product_type_id" id="product_type_id" class="form-control" value="{{ $rpd_filling_head->product->encrypt_product_type_id }}" readonly>
                                <input type="hidden" name="encrypt_id" id="encrypt_id" class="form-control" value="{{ $rpd_filling_head->encrypt_id }}" readonly>
                            </div>
                            <div class="form-group">
                                <label for="production_date">Production Date</label>
                                <textarea name="production_date" id="production_date" rows="3" class="form-control" readonly>@foreach ($rpd_filling_head->woNumbers as $wo_number){{ $wo_number->wo_number }} => {{ $wo_number->production_realisation_date }}&#13;&#10;@endforeach</textarea>
                            </div>
                            <div class="form-group">
                                <label for="sum_batch">&#x3A3; Batch</label>
                                <input type="text" name="sum_batch" id="sum_batch" value="{{ count($rpd_filling_head->woNumbers) }} Batch" class="form-control" readonly>
                            </div>
                        </div>
                    </div>
                    <div class="row">
...
@section('custom-plugin')
    ...
    <script>
        ...
        var table = $('#draft-sampel-table').DataTable({
            ...
            // "scrollX": true,
            ajax: {
                url: "{{ url('rollie/rpd-filling/get-draft-filling-sampel') }}",
        ...
        var table_done = $('#done-sampel-rpd-filling').DataTable({
            ...
            // "scrollX": true,
            ajax: {
                url: "{{ url('rollie/rpd-filling/get-done-filling-sampel') }}",
                dataSrc: function ( json )
                {
                    if(json.status!=='01' && json.status!='02') //!=false or !=token expire or !=token not valid
                    {
    ...
    <script>
        function addSampelPopUp()
        {
            ...
            $.ajax({
                url:"{{url('rollie/rpd-filling/add-filling-sampel-modal')}}",
                type:'POST',
                beforeSend:function()
                {
        ...
        function addBatch()
        {
            rpd_filling_head_id        = $('#encrypt_id').val();
            $.ajax({
                url:"{{url('rollie/rpd-filling/add-batch-modal')}}",
                type:'POST',
                beforeSend:function()
                {
                    $('.loading-bar').removeClass('hidden');
        ...
        {
            url     = "{{ url('rollie/rpd-filling/form') }}/"+rpd_filling_head_id;
        ...
        function analisa_sampel_pi(data)
        {
            ...
            event_sampel            = data[1];
            $.ajax({
                url:"{{url('rollie/rpd-filling/analisa-filling-sampel-modal')}}",
                type:'POST',
                beforeSend:function()
                {
                    $('.loading-bar').removeClass('hidden');
