        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
            <div class="card  card-outline">
                ...
                <div class="card-footer">
                    <div class="row">
                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                            <button class="btn btn-secondary" onclick="<?php if($data_draft == 0): ?> document.location.href='/rollie/production-schedule' <?php else: ?> backToDashboard() <?php endif; ?>" id="button-back-to-dashboard">
                                <i class="fas fa-arrow-left"> </i> Back To On Going Production Schedule
                            </button>
                            <button class="btn btn-primary <?php if($data_draft == 0): ?> hidden <?php endif; ?>" id="button-finalize-draft" onclick="finalizeDraft()">
...
<?php $__env->startSection('custom-plugin'); ?>
    <script>
        ...
        var draft_table = $('#draft-production-schedule-table').DataTable({
            ...
            // "scrollX": true,
            ajax: {
                url: "<?php echo e(url('rollie/production-schedule/get-data-draft')); ?>",
    ...
    <script>
        ...
        {
            $.ajax({
                url:"<?php echo e(url('rollie/production-schedule/upload-mtol-modal')); ?>",
                type:'POST',
                beforeSend:function()
                {
        ...
        {
            $.ajax({
                url:"<?php echo e(url('rollie/production-schedule/manual-add-modal')); ?>",
                type:'POST',
                beforeSend:function()
                {
                    $('.loading-bar').removeClass('hidden');
        ...
        function deleteDraftProductionSchedule(data)
        {
            ...
            Swal.fire({
                ...
                {
                    $.ajax({
                        url:"<?php echo e(url('rollie/production-schedule/remove-draft-schedule')); ?>",
                        type:'POST',
                        beforeSend:function()
                        {
                            $('.loading-bar').removeClass('hidden');
        ...
        function updateDraftProductionSchedule(data)
        {
            ...
            wo_id               = data[2];
            $.ajax({
                url:"<?php echo e(url('rollie/production-schedule/update-draft-schedule-modal')); ?>",
                type:'POST',
                beforeSend:function()
                {
                    $('.loading-bar').removeClass('hidden');
        ...
        function finalizeDraft()
        {
            Swal.fire({
                ...
                {
                    $.ajax({
                        url:"<?php echo e(url('rollie/production-schedule/finalize-draft-schedule')); ?>",
                        type:'POST',
                        beforeSend:function()
                        {
                            $('.loading-bar').removeClass('hidden');
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/sentul-integrated-system-stable/resources/views/rollie/production_schedule/new_production_schedule_form.blade.php ENDPATH**/ ?>